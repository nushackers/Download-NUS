...
module.exports = function(db, fsConfig, configDateFormat){
    ...
    function tryUploadingFile(file, shortid, dataset) {
        console.warn("uploading file directly is deprecated!");
        var d = Q.defer();
        if(!file || !file.size){
            d.resolve();
        } else {
    ...
    return {
        ...
        getUserDatasets: function(nusId){
            return User.find({
                        ...
                        include: [{
                            model: Dataset,
                            include: [User, DataCategory, DataType, DataFile]
                        }],
                        order: [ [ Dataset, 'updatedAt', 'DESC' ] ]
                }).then(function(user){
                    return {
        ...
        deleteDataset: function(id, user){
            var d = Q.defer();
            var userId = user.id;
            Dataset.find(id).success(function(dataset){
                if(user.level < 2 && dataset.UserId !== userId){
                    d.reject({
                        notAuthorized: true
                    });
                } else {
        ...
        uploadDataset: function(file, userId, categoryId, typeId, name, description){
            var d = Q.defer();
            Q.all([generateShortId(), wrapDeferred(User.find(userId)), wrapDeferred(DataCategory.find(categoryId)), wrapDeferred(DataType.find(typeId))])
            .spread(function(shortid, user, category, type) {
                var path = fsConfig.dir + shortid + '/';

                fs.mkdirSync(path);
                fs.renameSync(file.path, path + file.name);
